...
result_savepath = './log/result/result_OC/'
...
def main(args):
    ...
    try:
        checkpoint = torch.load('./log/temp_checkpoint/best_modelOC.pth')
        start_epoch = checkpoint['epoch']
        classifier.load_state_dict(checkpoint['model_state_dict'])
        log_string('Use pretrain model')
    except:
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        with torch.no_grad():
            ...
            if (instance_acc <= best_instance_acc):
                ...
                savepath = str(checkpoints_dir) + '/best_modelOC.pth'
                ...
                torch.save(state,result_savepath+'best_modelOC.pth')
            
            if(epoch % 10 == 0):
                logger.info('Save model...')
                savepath = './log/temp_checkpoint/latest_modelOC.pth'
                log_string('Saving at %s'% savepath)
                state = {
                    'epoch': epoch,
                    'instance_acc': instance_acc,
                ...
                torch.save(state, savepath)
                torch.save(state,result_savepath+'latest_modelOC.pth')
