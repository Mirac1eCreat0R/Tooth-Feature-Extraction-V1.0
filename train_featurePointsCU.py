...
result_savepath = './log/result/result_CU/'
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        with torch.no_grad():
            ...
            if (instance_acc <= best_instance_acc):
                ...
                savepath = str(checkpoints_dir) + '/best_modelCU.pth'
                ...
                torch.save(state,result_savepath+'best_modelCU.pth')
            
            if(epoch % 10 == 0):
                logger.info('Save model...')
                savepath = './log/temp_checkpoint/latest_modelCU.pth'
                log_string('Saving at %s'% savepath)
                state = {
                    'epoch': epoch,
                    'instance_acc': instance_acc,
                ...
                torch.save(state, savepath)
                torch.save(state,result_savepath+'latest_modelCU.pth')
